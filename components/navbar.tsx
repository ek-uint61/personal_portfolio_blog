  ...
  import { FaArrowLeft, FaBars, FaFolder, FaFolderOpen, FaFighterJet } from 'react-icons/fa';
      ...
      <nav className={`border-b-2 border-gray-600 fixed top-0 left-0 w-full z-10 transition-transform transform ${showNavbar ? 'translate-y-0' : '-translate-y-full'} bg-white`}>
        <div className="max-w-screen-xl flex items-center justify-between mx-auto p-3 ">
          <div className="flex justify-center w-full">
            <Link
              href="/"
              className="group font-semibold px-7 py-2 rounded-[9px] border-2 cursor-pointer transition-all duration-300 bg-white flex items-center gap-2 outline-none focus:scale-105 hover:bg-[#bab6b6] hover:border-6 hover:border-gray-100 dark:hover:bg-white active:scale-95 dark:bg-white/10 visited:bg-white visited:text-gray-700"
            >
                    ...
                    <ul className="pl-4 border-l-2 border-black ml-2">
                      {groupedPosts[category].map((post) => (
                        <li className={`text-sm font-semibold hover:underline  ${post.slug === currentPostSlug ? 'bg-[#b1b1b1b1] hover:bg-[#b4b4b4] border-2 border-black mt-2 mb-2  hover:no-underline rounded-md ' : ''}`} key={post.slug}>
                          <Link href={`/blog/${post.slug}`} className={`text-gray-600 hover:text-blue-700  text-xs ${post.slug === currentPostSlug ? 'text-gray-800 hover:text-gray-800 ' : ''}`} onClick={toggleMenu}>
